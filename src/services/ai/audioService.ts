...
${customInst}` : '';
  ...
  const prompt = `Ты - эксперт-музыкант и продюсер, специализирующийся на написании точных промптов для Suno AI.
На основе предоставленного сценария и темы видео, создай ОДИН подробный мастер-промпт (до 1000 символов) для генерации фоновой музыки, которая будет идеально подходить под настроение всего видео целиком. СТРОГО УЧИТЫВАЙ ТЕМУ И АУДИТОРИЮ ВИДЕО, СТРОГО СЛЕДУЙ ГЛОБАЛЬНЫМ НАСТРОЙКАМ ПРИЛОЖЕНИЯ (ГЛОБАЛЬНЫЕ ИНСТРУКЦИИ ПРИЛОЖЕНИЯ).
...
Опиши музыкальный жанр, инструменты, темп, настроение, ритмику, качество продакшена (например, cinematic, lo-fi, upbeat, corporate, ambient, synthwave, 120 bpm и т.д.).
Промпт должен быть написан на английском языке (так как Suno лучше понимает английский), представлять собой плотный набор ключевых слов и фраз через запятую, без лишних вступлений и пояснений. 
Строго уложись в лимит 1000 символов.${instructionsContext}`;
